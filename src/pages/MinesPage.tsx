...
const calc=(t:number,m:number,s:number)=>{ if(!s) return 1; const safe=t-m; if(s>safe) return 0; let p=1; for(let i=0;i<s;i++) p*=(safe-i)/(t-i); return p<=0?0:Number(((1/p)*0.97).toFixed(2)); };
        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-6">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="rounded-xl border border-[#252B38] bg-[#121620]/70 p-4"><p className="text-[10px] font-bold tracking-widest text-[#5F6878] flex items-center gap-1"><Trophy className="w-3 h-3"/> HOUSE EDGE</p><p className="mt-2 text-xs text-[#8B93A3]">3% - <span className="text-white font-mono">1 / P(safe)</span> server-side.</p></div>
