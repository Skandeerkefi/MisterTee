...
const ClashLeaderboardPage = () => {
  ...
  const calculatePeriod = () => {
    ...
    const start = new Date(
      baseDate.getTime() + intervals * 14 * 24 * 60 * 60 * 1000
    );

    const end = new Date(start.getTime() + 15 * 24 * 60 * 60 * 1000 - 1000);
  ...
  useEffect(() => {
    ...
    fetchLeaderboard(); // Backend determines the period internally
  }, [fetchLeaderboard]);
  ...
  // Sort players by wagered descending → highest wager = rank #1
  const sortedPlayers = [...players].sort(
    (a, b) => (b.wageredGems || 0) - (a.wageredGems || 0)
  );
      ...
      <main className="flex-grow container mx-auto p-4 relative z-10">
        ...
        {periodStart && periodEnd && (
          <p className="text-center text-white mb-4">
            Period: {formatDate(periodStart)} → {formatDate(periodEnd)} | Next update in:{" "}
            {countdown}
          </p>
        )}
            ...
            <table className="w-full text-left border-separate border-spacing-0 text-white shadow-xl rounded-xl overflow-hidden backdrop-blur-xl bg-white/5">
              <thead>
                <tr className="bg-gradient-to-r from-red-700 to-red-900 text-white">
                  <th className="p-3 border-b border-red-600 font-semibold text-center">
                    Rank
                  </th>
                  <th className="p-3 border-b border-red-600 font-semibold">
                    Username
                  </th>
                  <th className="p-3 border-b border-red-600 font-semibold text-center">
                    Wagered (Gems)
